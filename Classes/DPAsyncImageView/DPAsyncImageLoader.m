...
- (void)updateQueue
{
    @synchronized(self){
        //start connections
        NSMutableArray* tmpConnect = [self.connections mutableCopy];
        NSUInteger count = 0;
        for (DPAsyncImageConnection *connection in tmpConnect)
        {
...
- (void)imageLoaded:(NSNotification *)notification
{
    ...
    for (NSInteger i = (NSInteger)[self.connections count] - 1; i >= 0; i--)
    {
        ...
        if (connection.URL == URL || [connection.URL isEqual:URL])
        {
            ...
            {
                DPAsyncImageConnection *earlier = self.connections[(NSUInteger)j];
