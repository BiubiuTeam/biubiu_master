...
static const char kKeyChainUDIDAccessGroup[] = "8UN5C7NJAX.com.nuxsoft.biubiu";
//static const char kKeyChainUDIDAccessGroup[] = "3373JM3DR4.com.nuxsoft.biubiu";
...
+ (NSString *)getMacAddress
{
    ...
    DPTrace("Mac Address: %@", macAddressString);
...
+ (NSString*)getUDIDFromKeyChainExForOldVersion
{
    ...
    NSString *accessGroup = [NSString stringWithUTF8String:"3373JM3DR4.com.nuxsoft.biubiu"];
    if (accessGroup != nil)
    {
    ...
    if (queryErr == errSecItemNotFound) {
        DPTrace("KeyChain Item: %@ not found!!!", [NSString stringWithUTF8String:kKeychainUDIDItemIdentifier]);
    }
    else if (queryErr != errSecSuccess) {
        DPTrace("KeyChain Item query Error!!! Error code:%zd", queryErr);
    }
    if (queryErr == errSecSuccess) {
        DPTrace("KeyChain Item: %@", udidValue);
...
{
    NSMutableDictionary *dictForAdd = [[[NSMutableDictionary alloc] init] autorelease];
    ...
    if ([SvUDIDTools getUDIDFromKeyChain]) {        // there is item in keychain
        ...
        if (writeErr != errSecSuccess) {
            DPTrace("Add KeyChain Item Error!!! Error Code:%zd", writeErr);
            return NO;
        }
        else {
            DPTrace("Add KeyChain Item Success!!!");
            return YES;
        }
    }
    return NO;
...
+ (BOOL)removeUDIDFromKeyChain
{
    ...
    if (deleteErr != errSecSuccess) {
        DPTrace("delete UUID from KeyChain Error!!! Error code:%zd", deleteErr);
        [dictToDelete release];
        return NO;
    }
    else {
        DPTrace("delete success!!!");
    }
...
+ (BOOL)updateUDIDInKeyChain:(NSString*)newUDID
{
    ...
    if (queryResult) {
        ...
        if (updateErr != errSecSuccess) {
            DPTrace("Update KeyChain Item Error!!! Error Code:%zd", updateErr);
            ...
            DPTrace("Update KeyChain Item Success!!!");
            [dictForQuery release];
            [dictForUpdate release];
            return YES;
        }
